...
template<class _int, class _uint, class _ulong, _uint mod>
struct static_montgomery_mint {
	using mint = static_montgomery_mint<_int, _uint, _ulong, mod>;
	static_montgomery_mint() : _xr(0) {}
	static_montgomery_mint(_int x) : _xr(_reduce(_ulong(x%_int(mod)+mod)*_r2)) {}
	_uint val() const {
		_uint x = _reduce(_xr);
